...
public class GameView extends JPanel implements ActionListener{
	...
	private int lastMove = 0; // Track the last move: -1 (up), 0 (none), 1 (down)
	private boolean collectData = false; // Flag to enable/disable data collection
	
	private NerualNetwork nn = new NerualNetwork();
	...
	private void initModel() {
		...
		//create and train when game is built
		nn.createNetwork();
		nn.loadAndTrain();
		nn.saveNetwork();
	}
	...
	public void move(int step) {
	    //update player postion for data collection
	    playerRow += step;
	    
	    //check if data collection is enabled
	    if (collectData) {
	        //extract the features for data collection we need the model(the cave), player position and the last move made by the player
	    	double[] features = DataCollection.extractFeatures(model, playerRow, lastMove); 
	    	DataCollection.saveToCSV(features, step); // we save these features to a csv file with last move being the label
	    }
	    //storing last move for collection
	    lastMove = step;
	}
	...
	public void toggleDataCollection() {
	    ...
	    System.out.println("Data collection " + (collectData ? "started" : "stopped"));
	}
	...
	private void autoMove() {
		...
		//get a decison which is just the AIs prediction on what move to make based on features provided
		int decision = nn.predictMove(model, playerRow, lastMove);
		//log the AIs decision for debugging purposes
		System.out.println("AI decision: " + decision);
		
		//rules based bot to play the game and collect data optimised to collect good data
		int botDecision = Bot.decideMove(model, playerRow, lastMove);
		
		//make a move based on decision of AI
		move(decision);
		
		//allow the bot to move
		//move(botDecision)
	}
